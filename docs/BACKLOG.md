...
| 16 | Damage / health model | **DONE (sim)** | Phase 3. `src/sim/health.ts` (pure HP), `src/sim/combat.ts` (contact dispatch, damage tuning), cart death → stroke penalty → respawn. Shape only was taken from the reference `damage.ts`; no code. See `docs/superpowers/specs/2026-09-02-targets-health-combat-design.md` and `docs/superpowers/plans/2026-09-02-targets-health-combat-implementation.md`. The HUD health bar those concept shots show now has a model — see `docs/superpowers/specs/2026-09-02-combat-rendering-hud-design.md` and `docs/superpowers/plans/2026-09-02-combat-rendering-hud-implementation.md`. |
...
| 16c | Cart cosmetics: turret skin, chassis paint, tire type | **PART DONE** / **3.5** (UI) | Phase 2 landed `TIRE_TUNING` as a real trade (turf fastest on fairway, worst in sand; knobby the reverse). Turret skin and chassis paint are still unbuilt — no reader and no UI until 3.5. The rider is deliberately **not** paintable through this: his four slots (`skin`/`shirt`/`trousers`/`cap`) share no names with the cart's eight, so a chassis repaint cannot reach him. A rider cosmetic would be its own category. |
| 16e | Cart rider | **DONE** | Blender primitive graph, `src/entities/graphs/driver.json`. 15 parts, seated at the wheel, four own slots. Amends `ASSET_PIPELINE.md` §2.2, which rules the *ragdoll target* mannequin procedural because its physics rig is its character rig — the rider has no rigid bodies, so none of that reasoning reaches him. **Invisible from the default chase camera**: the canopy is at y 2.005 and the camera sits 3.6 m up, so he only reads on bot carts, in the clubhouse turntable and at oblique angles. Lowering `CHASE_HEIGHT` toward concept 03's framing is the fix, and is a camera-feel decision, not an art one. |
| 16f | The swing animation | **DONE** | `GolfClub.setSwing(charge01, reload01)` over `swing_yoke`/`swing_arm`, both authored *below* `barrel_pitch` so swing angle 0 is a no-op on the muzzle. No new sim state: both inputs already existed. One thing is recorded rather than solved: the ball leaves at the top of the backswing rather than at impact (the sim fires on the release edge; closing it means input latency). The flat swing plane it shipped with is **fixed** — see 16g. See `art/README.md`. |
| 16g | Turret geometry and the swing plane | **DONE** | `docs/superpowers/specs/2026-09-08-turret-geometry-and-swing-plane-design.md`, implemented whole. `TURRET_GEOMETRY.pivotHeight` moved 2.05 → 2.60 and now names `barrel_pitch` rather than the yaw ring, which closes the 0.26 m muzzle defect **by construction**; `pivotForward` 0.45 is new and is why `computeMuzzle` reads `heading` as well as `turretYaw`. A 0.55 m pedestal buys a near-vertical swing plane (0.25 rad, was 1.15), the housing pitches back on its trunnion, the club heads gained hosels and heel mounting, and the rider gained ball joints, a neck and fists. The binding constraint turned out to be the **putter**, not the driver — 3° of loft reaches the roof first. |
| 16d | Cart-mode `sim.ballLoaded`/`view.ballLoaded` reads are stale | **DONE** | Fixed by deleting rather than rewiring: `ballLoaded` and `ballInReach` are gone from the codebase entirely, not patched to read the new mechanic. `FrameView.turretLoaded` replaces them, derived straight from `cart.ammo` (`sim.mode === SwingMode.Cart && sim.cart.ammo > 0`) — the stale flag wasn't just outdated, it was actively harmful, since it kept describing a "drive over the ball to load it" mechanic the ammo fork had already replaced. See `src/main.ts`'s `turretLoaded()` and `src/render/scene.ts`'s `FrameView.turretLoaded`. |
...
| 24a | **Knockable targets / ragdolls** | **DONE** | Phase 3. `src/sim/entities/Target.ts`: eleven capsules held rigid by body type, whole rig flipped to Dynamic on a ball contact, revolute limits on the created joint, self-collision off via groups, post-impact velocity clamped. Three are hardcoded along the tee→cup corridor, the same interim placement the ammo bucket uses. No auto-recovery: a target stays down until the next hole. Ragdolls now render — the sim-only gap is closed, see `docs/superpowers/specs/2026-09-02-combat-rendering-hud-design.md` and `docs/superpowers/plans/2026-09-02-combat-rendering-hud-implementation.md`. |
