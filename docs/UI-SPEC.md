...
| H8 | Minimap | 02, 03, 08, 09, 14 | 4 | `surfaceAt`, entity positions | **Built.** `M` cycles closed -> this hole -> the whole course -> closed, `Escape` closes from anywhere; `src/ui/courseMap.ts` draws it and `src/ui/mapCamera.ts` frames it, both from `src/sim/mapGeometry.ts` -- the same functions the committed hole plans are drawn from. In *every* in-game shot; the single most under-planned element in the old roadmap. Top-right, rounded. Renders from `surfaceAt` alone — no authored map data, so nothing to keep in sync (same argument as `surfaces.ts`). Shows hole shape, own heading arrow, and in multiplayer a dot per cart (09). |
...
| H13 | Cart nameplates | 07, 09 | 4 | remote cart positions | **Built**, less the health bar. Rules in `src/ui/plateState.ts`: distance exact under 100 m, rounded to 25 and marked approximate to 300 m, no number past it; allies render through terrain, enemies only in line of sight and fade over a second once it breaks. Floating pill above each cart: team-coloured name, plus a health bar in 09. Reuses H11's projection but persists per-frame rather than being one-shot — budget for that difference. |
