...
**Four stacked PRs are open, in this order. Nothing is merged.**

| PR | Branch | What |
|---|---|---|
| [#9](https://github.com/NCSTATEPACK16/TeeTimeTurrets/pull/9) | `primitive-graph-and-cart` | The Blender primitive-graph pipeline and the 47-object cart |
| [#10](https://github.com/NCSTATEPACK16/TeeTimeTurrets/pull/10) | `phase-1-75-screens` | Phase 1.75: title, round, results, scorecard |
| [#11](https://github.com/NCSTATEPACK16/TeeTimeTurrets/pull/11) | `clubhouse-and-economy` | The clubhouse, the first decorative GLB, the purse |
| [#12](https://github.com/NCSTATEPACK16/TeeTimeTurrets/pull/12) | `fix-round-advance` | One round per session; a purse that prices one hole |

Each is based on the one above it, so **#9 merges first** and the rest rebase down. `main` is at
`289e683` and has none of it.

**Verified at the tip (`fix-round-advance`):** `tsc` clean · 575 tests / 38 files · `npm run build`
gate 5/5 mean delta 0.00 · `npm run smoke` PASS including the Phase 1.75 memory gate, the clubhouse
memory gate and the new hole-advance check · `npm run plan` byte-identical · `npm run probe` red on
the one known driver-distance line only.
