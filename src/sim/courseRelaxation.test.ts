...
describe("relaxNine", () => {
  it("never changes a hole's own length", () => {
    ...
    const relaxed = relaxNine(holes, initial, hub, clubhouse, 1);
    for (const h of holes) {
      const local = dist(h.tee, h.cup);
      const placed = dist(place(relaxed, h.index, h.tee), place(relaxed, h.index, h.cup));
  ...
  it("pulls the last cup closer to the clubhouse than the bad initial guess left it", () => {
    const { holes, initial, hub, clubhouse } = straightLine();
    const before = dist(place(initial, 2, holes[2]!.cup), clubhouse);
    const relaxed = relaxNine(holes, initial, hub, clubhouse, 1);
    const after = dist(place(relaxed, 2, holes[2]!.cup), clubhouse);
  ...
  it("keeps every transition within the slack bounds", () => {
    ...
    for (let i = 0; i + 1 < holes.length; i++) {
      const green = place(relaxed, i, holes[i]!.cup);
      const tee = place(relaxed, i + 1, holes[i + 1]!.tee);
  ...
  it("pins the first hole's tee on the hub", () => {
    const { holes, initial, hub, clubhouse } = straightLine();
    const relaxed = relaxNine(holes, initial, hub, clubhouse, 1);
    expect(dist(place(relaxed, 0, holes[0]!.tee), hub)).toBeLessThan(0.01);
  });
});
